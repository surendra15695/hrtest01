...
@Component({
  selector: 'app-archivedjob',
  templateUrl: './archivedjob.component.html',
  styleUrls: ['./archivedjob.component.css']
})
export class ArchivedjobComponent implements OnInit {
  pageTitle: string = "Archived Jobs";
  ...
  getVendorJobList(param) {
    ...
    this.vendorJobService.getVendorArchivedJobList(this.searchform.value).subscribe((result) => {
      if (result) {
  ...
  getAllFunction() {
    this.functions = [];
    this.searchFunction.verticalId = this.searchform.value.verticalId;
  ...
  getAllPosition() {
    this.positions = [];
    this.searchPosition.verticalId = this.searchform.value.verticalId;
  ...
  gotoSubmittedCandidate(id) {
    this.persistance.set('pagename', "archivedjobs");
    this.persistance.set('paramid', id);
