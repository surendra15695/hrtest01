...
@Component({
  selector: 'app-hand-holding-access',
  templateUrl: './hand-holding-access.component.html',
  styleUrls: ['./hand-holding-access.component.css']
})
export class HandHoldingAccessComponent implements OnInit {
  ...
  onChangeUserRole(roleId: any) {
    ...
    if ((this.SelUserRole == 60) || (this.SelUserRole == 63)||(this.SelUserRole == 35)||(this.SelUserRole == 57)||
        (this.SelUserRole == 44)||(this.SelUserRole == 45)||(this.SelUserRole == 46)||
        (this.SelUserRole == 21)||(this.SelUserRole == 22)||(this.SelUserRole == 23)) {
  ...
  onSubmit(){
    var locations=[]
    locations=this.str.split(",")
    if(locations.includes("0")){
      this.str="";
      for(var val of this.locations){
        this.str += val.locationId.toString() +","
      }
      this.str=this.str.slice(2,-1);
    }
    if((this.saveForm.value.roleId== 60) || (this.saveForm.value.roleId == 63)||(this.saveForm.value.roleId == 35)||(this.saveForm.value.roleId == 57)||
       (this.saveForm.value.roleId == 44)||(this.saveForm.value.roleId == 45)||(this.saveForm.value.roleId == 46)||
       (this.saveForm.value.roleId == 21)||(this.saveForm.value.roleId == 22)||(this.saveForm.value.roleId == 23)){
      this.record = {
      ...
      if(this.areaArray.length >0){
        for(var valarea of this.areaArray){
          areas += valarea+","
        }
  ...
  onEdit(RowData: any) {
    ...
    this.areaArray=[]
    this.areaArray=RowData.areaName.split(",");
    this.saveForm.patchValue({
  ...
  onAddArea(){
    var flag=0;
    if(this.areaName.includes(",")){
      this.notiService.showError("You cann't Enter Comma, Please remove comma","Error");
      flag=1
    }
    if(flag==0){
    this.areaArray.push(this.areaName.trim());
    this.areaName="";
    }
  }
  onDeleteArea(data){
    this.areaArray=this.areaArray.filter( e=> e.trim() != data.trim());
  }
}
class Function {
  LocationId: number;
