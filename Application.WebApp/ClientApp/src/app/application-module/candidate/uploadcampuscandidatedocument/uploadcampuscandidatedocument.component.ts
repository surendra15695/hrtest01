...
import {
  ...
  IAttachedDocument, IAttachedDocumentArrayList, IOfferDocumentRemarksDetails, ISubmittedAttachedDocumentArrayList
...
@Component({
  selector: 'app-uploadcampuscandidatedocument',
  templateUrl: './uploadcampuscandidatedocument.component.html',
  styleUrls: ['./uploadcampuscandidatedocument.component.css']
})
export class UploadcampuscandidatedocumentComponent implements OnInit {
  ...
  addNewSalary() {
    ...
    // console.log(this.previousNewSalary);
  ...
  submitData(status) {
    if (this.documentData.attachmentDetails.length == 0 && this.attachedDocumentArrayList.length == 0) {
      ...
      this.documentService.campusupdateCandidateOfferDocument(formData).subscribe((result) => {
        // console.log(result);
