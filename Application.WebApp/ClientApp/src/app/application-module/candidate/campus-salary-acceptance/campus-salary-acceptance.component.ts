...
@Component({
  selector: 'app-campus-salary-acceptance',
  templateUrl: './campus-salary-acceptance.component.html',
  styleUrls: ['./campus-salary-acceptance.component.css']
})
export class CampusSalaryAcceptanceComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (flag == 0) {
      ...
      this.salaryFitmentService.acceptRejectCampusSalaryFitment(formData).subscribe((result) => {
        if (result) {
          console.log(result);
