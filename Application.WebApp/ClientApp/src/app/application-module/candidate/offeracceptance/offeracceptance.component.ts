...
@Component({
  selector: 'app-offeracceptance',
  templateUrl: './offeracceptance.component.html',
  styleUrls: ['./offeracceptance.component.css']
})
export class OfferacceptanceComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (flag == 0) {
      ...
      this.offerLetterService.acceptRejectOffer(formData).subscribe((result) => {
        if (result) {
          // console.log(result);
