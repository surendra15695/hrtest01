...
@Component({
  ...
  styleUrls: ['./edit-registration-form.component.css']
})
export class EditRegistrationFormComponent implements OnInit {
  ...
  getRegistrationDetails(){
    ...
    this.requisitionService.getCandidateRegistrationDetail(obj).subscribe((response: any) => {
      if (response) {
        ...
        this.campusCandidate.HighestQualification=Number(response.profileData[0].highestQualification);
        // console.log("profile data",this.campusCandidate)
        // console.log("chck",response.otherAcademicData,response.otherAcademicData.length)
        if(response.otherAcademicData.length>0)
        {
          this.campusCandidate.AnyOtherQualification=response.otherAcademicData[0].qualificationName;
          this.campusCandidate.AnyOtherQualificationCourse=response.otherAcademicData[0].courseName;
          this.campusCandidate.AnyOtherQualificationCourseStatus=Number(response.otherAcademicData[0].courseStatus);
          this.campusCandidate.AnyOtherQualificationStream=response.otherAcademicData[0].specalization;
          this.campusCandidate.AnyOtherQualificationInstitite=Number(response.otherAcademicData[0].instutation);
          this.campusCandidate.AnyOtherQualificationInstititeName=response.otherAcademicData[0].instutationName;
          this.campusCandidate.AnyOtherQualificationInstititeLocation=response.otherAcademicData[0].instutationLocation;
          this.campusCandidate.AnyOtherQualificationYearOfPassing=response.otherAcademicData[0].yearOfPassing;
          this.campusCandidate.AnyOtherQualificationMarks=response.otherAcademicData[0].marks;
          if(this.campusCandidate.AnyOtherQualificationInstitite==0)
          {
            this.hideanyotherinstitute=false;
          }
        }
        for(let i=0;i<response.academicData.length;i++)
        {
          if(response.academicData[i].visualOrder==1)
          {
            this.campusCandidate.TenCourses=Number(response.academicData[i].course);
            this.campusCandidate.TenCourseStatus = Number(response.academicData[i].courseStatus);
            this.campusCandidate.TenYearOfPassing= response.academicData[i].yearOfPassing;
            this.campusCandidate.TenMarks=response.academicData[i].marks;
          }
          else if(response.academicData[i].visualOrder==2)
          {
            this.campusCandidate.TwelveCourses=Number(response.academicData[i].course);
            this.campusCandidate.TwelveCourseStatus = Number(response.academicData[i].courseStatus);
            this.campusCandidate.TwelveYearOfPassing= response.academicData[i].yearOfPassing;
            this.campusCandidate.TwelveMarks=response.academicData[i].marks;
          ...
          else if(response.academicData[i].visualOrder==3)
          {
            this.campusCandidate.DiplomaCourses=Number(response.academicData[i].course);
            this.getAllDiplomaStream(this.campusCandidate.DiplomaCourses);
            this.campusCandidate.DiplomaCourseStatus = Number(response.academicData[i].courseStatus);
            this.campusCandidate.DiplomaYearOfPassing= response.academicData[i].yearOfPassing;
            this.campusCandidate.DiplomaMarks=response.academicData[i].marks;
            this.campusCandidate.DiplomaStreams=Number(response.academicData[i].stream);
            this.campusCandidate.DiplomaUniversity=Number(response.academicData[i].instutation);
            this.campusCandidate.DiplomaInstituteName=response.academicData[i].instutationName;
            this.campusCandidate.DiplomaInstituteLocation=response.academicData[i].instutationLocation;
            if(this.campusCandidate.DiplomaCourseStatus==3)
            {
            ...
            if(this.campusCandidate.DiplomaUniversity==0)
            {
              this.hidediplomainstitutename=false;
            }
          }
          else if(response.academicData[i].visualOrder==4)
          {
            this.campusCandidate.DegreeCourses=Number(response.academicData[i].course);
            this.getAllDegreeStream(this.campusCandidate.DegreeCourses);
            this.campusCandidate.DegreeCourseStatus = Number(response.academicData[i].courseStatus);
            this.campusCandidate.DegreeYearOfPassing= response.academicData[i].yearOfPassing;
            this.campusCandidate.DegreeMarks=response.academicData[i].marks;
            this.campusCandidate.DegreeStreams=Number(response.academicData[i].stream);
            this.campusCandidate.DegreeUniversity=Number(response.academicData[i].instutation);
            this.campusCandidate.DegreeInstituteName=response.academicData[i].instutationName;
            this.campusCandidate.DegreeInstituteLocation=response.academicData[i].instutationLocation;
            if(this.campusCandidate.DegreeCourseStatus==3)
            {
            ...
            if(this.campusCandidate.DegreeUniversity==0)
            {
              this.hideunderinstitute=false;
            }
          }
          else if(response.academicData[i].visualOrder==5)
          {
            this.campusCandidate.PostDegreeCourses=Number(response.academicData[i].course);
            this.getAllPostDegreeStream(this.campusCandidate.PostDegreeCourses);
            this.campusCandidate.PostDegreeCourseStatus = Number(response.academicData[i].courseStatus);
            this.campusCandidate.PostDegreeYearOfPassing= response.academicData[i].yearOfPassing;
            this.campusCandidate.PostDegreeMarks=response.academicData[i].marks;
            this.campusCandidate.PostDegreeStreams=Number(response.academicData[i].stream);
            this.campusCandidate.PostDegreeUniversity=Number(response.academicData[i].instutation);
            this.campusCandidate.PostDegreeInstituteName=response.academicData[i].instutationName;
            this.campusCandidate.PostDegreeInstituteLocation=response.academicData[i].instutationLocation;
            if(this.campusCandidate.PostDegreeCourseStatus==3)
            {
            ...
            if(this.campusCandidate.PostDegreeUniversity==0)
            {
              this.hidepginstitute=false;
            }
          }
        }
        this.changeQualification(this.campusCandidate.HighestQualification);
      }
    })
  }
  // getAllCampusLink() {
  ...
  isvalid(){
    ...
    else if(this.campusCandidate.ExtraCurricularActivities==null||this.campusCandidate.ExtraCurricularActivities==undefined||this.campusCandidate.ExtraCurricularActivities==""){
  ...
  chck()
  {
    console.log("len",this.campusCandidate.TenMarks)
