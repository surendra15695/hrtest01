...
import { ISearchCandidateData } from 'src/app/interfaces/candidate/candidate.interface';
import { ICandidateAcademicDetails, ICandidateAcademicDetailsArray, ICandidateCertificationDetails, ICandidateCertificationDetailsArray, ICandidateExtraCarricularActivitiesDetails, ICandidateExtraCarricularActivitiesDetailsArray, ICandidateFamilyDetails, ICandidateFamilyDetailsArray, ICandidateLanguageKnownDetails, ICandidateLanguageKnownDetailsArray, ICandidateMembershipDetails, ICandidateMembershipDetailsArray, ICandidateMRFRelatives, ICandidateMRFRelativesArray, ICandidatePeviousAssignmentDetails, ICandidatePeviousAssignmentDetailsArray, ICandidateProfile, ICandidateTyreRelatives, ICandidateTyreRelativesArray, IMRFPreInterviewArray, IMRFPreInterviewDetails, ISearchCandidateProfile } from 'src/app/interfaces/candidate/candidateprofile.interface';
...
import { IDropDown, IState } from 'src/app/interfaces/common/common.interface';
import { ICountry, ISearchCountry } from 'src/app/interfaces/common/country.interface';
import { IQualificationCourse, ISearchQualificationCourse } from 'src/app/interfaces/common/course.interface';
...
import { IQualification, IQualificationType, IQulificationClassGaradeDivision, ISearchQualification, ISearchQulificationClassGaradeDivision } from 'src/app/interfaces/common/qualification.interface';
...
import { ISearchVendor, IVendor } from 'src/app/interfaces/vendor/vendor.interface';
...
import { CommondataService } from 'src/app/services/common/commondata/commondata.service';
...
@Component({
  selector: 'app-mrfppf',
  templateUrl: './mrfppf.component.html',
  styleUrls: ['./mrfppf.component.css']
})
export class MRFPPFComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.persistance.get('pagename') == "htverifydocument")
    {
      this.candidateId = this.persistance.get('CandidateID');
      this.createdby = this.persistance.get('CreatedBy');
    }
  }
  ...
  getAllFamilyOccupations() {
    ...
    this.commonService.getAllOccupation(this.searchOccupation).subscribe((result) => {
      if (result) {
        ...
        // console.log("Occupation List", this.familyOccupations);
  ...
  getCandidateProfile() {
    ...
    this.candidateService.getCandidateProfileApplication(this.searchCandidateProfile).subscribe((result) => {
      if (result) {
        //console.log("Candidate Profile Details", result);
        ...
        if (this.candidateProfile.isEnabled == false) {
          ...
          // this._route.navigate(['/candidate-application/view']);
        }
  ...
  addFamily() {
    ...
    if (this.arr.length != 0) {
     ...
     // console.log("find2", this.arr[count - 1].familyRelationShip)
      for (let i = 0; i < this.arr.length; i++) {
        if (this.arr[i].familyRelationShip == this.familyRelationshipId) {
    ...
    if (flag == 0) {
      ...
      // console.log("chck",this.familyRelationshipId)
  ...
  //   this.languageKnownDetails.splice(index, 1);
  //   console.log("find",this.languageKnownDetails)
  // }

  addmother() {
    this.motherarr = [];
   // console.log("chck", this.motherspeak);
    var flag = 0;
    ...
    //  console.log("try", this.languageKnownDetails)
     ...
     // console.log("chck", this.languageKnownDetails)
      ...
      // this.languageKnownDetails.shift();
      //console.log("New ", this.languageKnownDetails);
    }
  ...
  SaveApplicationForm(submitStatus) {
    ...
    if (flag == 0) {
      ...
      formData.append("PermanentPhone", this.candidateProfile.permanentPhone.toString());
      ...
      // console.log(JSON.stringify(this.preInterviewArray));
      ...
      this.SpinnerService.show();
      this.candidateService.CandidateProfileMrfPpf(formData).subscribe((result) => {
