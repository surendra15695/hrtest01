...
@Component({
  selector: 'app-campus-interview-reimbursement-list',
  templateUrl: './campus-interview-reimbursement-list.component.html',
  styleUrls: ['./campus-interview-reimbursement-list.component.css']
})
export class CampusInterviewReimbursementListComponent implements OnInit {
  ...
  ngOnInit() {
    console.log("ver",this.persistance.get('loggedinuser'));
    this.loadDatePicker();
    this.loadDataTable1();
    this.loadDataTable2();
  ...
  fromPendingSubmit() {
    ...
    this.travelReimbursementService.getCampusTravelReimbursementDetailList(this.searchpendingform.value).subscribe((result) => {
      if (result) {
        ...
        this.pendingTravelReimbursementList=result.filter(e=>(e.isOffCampus==0));
  ...
  fromProcessedSubmit() {
    ...
    this.travelReimbursementService.getCampusTravelReimbursementDetailList(this.searchprocessedform.value).subscribe((result) => {
      if (result) {
        if(this.verticalname=="sales")
        {
          result = result.filter(e => ((e.campusForId=="2") && (e.hrOpsAssigned != "1")))
        ...
        this.processedTravelReimbursementList = result.filter(e=>(e.isOffCampus==0));;
  ...
  onclickassigntorec(interviewDetailId) {
    jQuery(".custom-menu").hide();
    this.interviewDetailIds = interviewDetailId.toString();
  }
  onassign(){
    debugger;
    console.log("rec",this.recruitmentautouser)
    console.log('id',this.interviewDetailIds)
    let obj =
    {
      InterviewDetailIds : this.interviewDetailIds,
      AutoUserId : this.recruitmentautouser
    }
    this.travelReimbursementService.AssignInterviewTravel(obj).subscribe((result) => {
      if (result) {
        if (result.successFlag == 0) {
          this.notificationService.showError(result.msg, "Error");
        }
  ...
  gotoViewDetail(interviewDetailId, candidateNo) {
    jQuery(".custom-menu").hide();
    ...
    this._route.navigate(['/app/interview-reimbursement/view']);
  }
