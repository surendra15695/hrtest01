...
@Component({
  selector: 'app-recruitmentinterviewreimbursementlist',
  templateUrl: './recruitmentinterviewreimbursementlist.component.html',
  styleUrls: ['./recruitmentinterviewreimbursementlist.component.css']
})
export class RecruitmentinterviewreimbursementlistComponent implements OnInit {
  ...
  fromPendingSubmit() {
    ...
    this.travelReimbursementService.getCampusTravelReimbursementDetailList(this.searchpendingform.value).subscribe((result) => {
      if (result) {
        result = result.filter(e => ((e.hrOpsAssigned=="1") && (e.hrOpsAssignedAutoUserId == this.autoUserId.toString())))
