...
@Component({
  selector: 'app-plantemployeemanagement',
  templateUrl: './plantemployeemanagement.component.html',
  styleUrls: ['./plantemployeemanagement.component.css']
})
export class PlantemployeemanagementComponent implements OnInit {
  ...
  pagValuePlant:number;
  displaystart:number;
  previousValuesPlant:any = {};
  constructor(
    ...
    if(this.persistance.get('previouspageparamsP') != (null || undefined)){
      var params=this.persistance.get('previouspageparamsP');
  ...
  ngOnInit() {
    ...
    this.persistance.get('tabledisplayStartP') == (null || undefined) ? this.displaystart=0 : this.displaystart=this.persistance.get('tabledisplayStartP');
    if(this.persistance.get('tabledisplayStartP') ==(null || undefined)){
      this.displaystart=0
    }
    if(this.persistance.get('tabledisplayStartP') >0){
      var tablestart=this.persistance.get('tabledisplayStartP')
  ...
  loadDataTable() {
    ...
    setTimeout(() => {
      ...
      jQuery('#dataTable1').dataTable({
        ...
        dothis.pagValuePlant=(settings._iDisplayStart/settings._iDisplayLength) +1
  ...
  onClickNextPage(){
    this.previousValuesPlant.requisitionDetailId = this.searchFormEmployeeManagement.value.requisitionDetailId;
    this.previousValuesPlant.candidateId = this.searchFormEmployeeManagement.value.candidateId;
    this.previousValuesPlant.employeeNo = this.searchFormEmployeeManagement.value.employeeNo;
    this.previousValuesPlant.requisitionNo = this.searchFormEmployeeManagement.value.requisitionNo;
    this.previousValuesPlant.source = this.searchFormEmployeeManagement.value.source;
    this.previousValuesPlant.name = this.searchFormEmployeeManagement.value.name;
    this.previousValuesPlant.hiringStatus = this.searchFormEmployeeManagement.value.hiringStatus;
    this.previousValuesPlant.employeeStatus = this.searchFormEmployeeManagement.value.employeeStatus;
    this.previousValuesPlant.dtofJoiningFrom = this.searchFormEmployeeManagement.value.dtofJoiningFrom;
    this.previousValuesPlant.dtofJoiningTo = this.searchFormEmployeeManagement.value.dtofJoiningTo;
  ...
  getAllEmployee() {
    ...
    this.recruitmentManagerService.getAllEmployeeManagementList(this.searchFormEmployeeManagement.value).subscribe((result) => {
      if (result) {
        ...
        this.employeeManagementList = this.employeeManagementList.filter(e => ((e.verticalId == 2)&&(e.hiringStatusId==54)));
        console.log("check",this.employeeManagementList)
  ...
  onClickViewDocument(data) {
    ...
    this.persistance.set('previouspagefiltersP',this.previousValuesPlant);
    this.persistance.set('tabledisplayStartP',this.pagValuePlant);
    jQuery(".custom-menu").hide();
