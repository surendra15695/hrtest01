...
@Component({
  selector: 'app-corporateemployeemanagement',
  templateUrl: './corporateemployeemanagement.component.html',
  styleUrls: ['./corporateemployeemanagement.component.css']
})
export class CorporateemployeemanagementComponent implements OnInit {
  ...
  pagValueCorporate:number;
  displaystart:number;
  previousValuesCorporate:any = {};
  constructor(
    ...
    if(this.persistance.get('previouspageparamsC') != (null || undefined)){
      var params=this.persistance.get('previouspageparamsC');
  ...
  ngOnInit() {
    this.persistance.get('tabledisplayStartC') == (null || undefined) ? this.displaystart=0 : this.displaystart=this.persistance.get('tabledisplayStartC');
    if(this.persistance.get('tabledisplayStartC') ==(null || undefined)){
      this.displaystart=0
    }
    if(this.persistance.get('tabledisplayStartC') >0){
      var tablestart=this.persistance.get('tabledisplayStartC')
    ...
    setTimeout(() => {
      ...
      jQuery('#dataTable1').dataTable({
        ...
        "drawCallback": function (settings) {
         dothis.pagValueCorporate=(settings._iDisplayStart/settings._iDisplayLength) +1
  ...
  onClickNextPage(){
    this.previousValuesCorporate.requisitionDetailId = this.searchFormEmployeeManagement.value.requisitionDetailId;
    this.previousValuesCorporate.candidateId = this.searchFormEmployeeManagement.value.candidateId;
    this.previousValuesCorporate.employeeNo = this.searchFormEmployeeManagement.value.employeeNo;
    this.previousValuesCorporate.requisitionNo = this.searchFormEmployeeManagement.value.requisitionNo;
    this.previousValuesCorporate.source = this.searchFormEmployeeManagement.value.source;
    this.previousValuesCorporate.name = this.searchFormEmployeeManagement.value.name;
    this.previousValuesCorporate.hiringStatus = this.searchFormEmployeeManagement.value.hiringStatus;
    this.previousValuesCorporate.employeeStatus = this.searchFormEmployeeManagement.value.employeeStatus;
    this.previousValuesCorporate.dtofJoiningFrom = this.searchFormEmployeeManagement.value.dtofJoiningFrom;
    this.previousValuesCorporate.dtofJoiningTo = this.searchFormEmployeeManagement.value.dtofJoiningTo;
  ...
  getAllEmployee() {
    ...
    //console.log("Search Corporate Obj", this.searchFormEmployeeManagement.value);
    ...
    this.recruitmentManagerService.getAllEmployeeManagementList(this.searchFormEmployeeManagement.value).subscribe((result) => {
      if (result) {
        ...
        this.employeeManagementList=this.employeeManagementList.filter(e=>((e.verticalId==1)&&(e.hiringStatusId==54)));
  ...
  btnClickRelocationReimbursement() {
    ...
    this.employeeManagementList.forEach(element => {
      if (element.checked) {
        candidateNoString += (cflag == 0 ? "" : ", ") + element.empNo;
        cflag = 1;
      }
    })
    this.employeeId = candidateNoString;
  ...
  onClickViewDocument(data) {
    ...
    this.persistance.set('previouspagefiltersC',this.previousValuesCorporate);
    this.persistance.set('tabledisplayStartC',this.pagValueCorporate);
    jQuery(".custom-menu").hide();
