...
@Component({
  selector: 'app-occorporatediscontinuedcandidatelist',
  templateUrl: './occorporatediscontinuedcandidatelist.component.html',
  styleUrls: ['./occorporatediscontinuedcandidatelist.component.css']
})
export class OccorporatediscontinuedcandidatelistComponent implements OnInit {
  ...
  getAllLocation() {
    this.locations = [];
    this.searchLocation.verticalId = 1;
  ...
  getAllDiscontinuedCandidate() {
    ...
    this.hiringteamService.getAllDiscontinuedCandidateList(this.searchform.value).subscribe((result) => {
      if (result) {
       ...
       // console.log("Discontinued Candidate list", this.discontinuedCandidateList);
        this.SpinnerService.hide();
      }
      else {
  ...
  onClickReassignCandidate(data) {
    ...
    if(data.batchId!=0 && data.batchId!=null){
      this._route.navigate(['/app/oc-reassign-candiadte'], { queryParams: { CandidateId: data.candidateId, BatchId: data.batchId, BatchNo: data.batchNo, For: "Candidate", Type: 'C', From: 'DCL',  ReqDetailsId: data.requisitionDetailId, EmpNo: data.empployeeNo } });
    }else{
      this._route.navigate(['/app/oc-reassign-individualcandiadte'], { queryParams: { CandidateId: data.candidateId, BatchId: data.batchId, BatchNo: data.batchNo, For: "Candidate", Type: 'C', From: 'DCL',  ReqDetailsId: data.requisitionDetailId, EmpNo: data.empployeeNo } });
    }
  }
