...
@Component({
  selector: 'app-ocreassigncandidate',
  templateUrl: './ocreassigncandidate.component.html',
  styleUrls: ['./ocreassigncandidate.component.css']
})
export class OcreassigncandidateComponent implements OnInit {
  ...
  // By Anif on 21-02-2023 from here
  ...
  // By Anif on 21-02-2023 end here
  constructor(
    ...
    this.objEditSchedule = new EditSchedule();     // By Anif on 21-02-2023               
    ...
    this.trainerRoleid = 47;                       // Added by aif on 21-02-2023 this page will come only for batch
    ...
    if (this.candidateId != undefined && this.batchId == undefined) {
      ...
      this.getCandidateInfoIndividual();
      this.showBatchSection = false;
      this.reassigningFor = "Candidate";
      //this.trainerRoleid = 48;                  // By anif on 21-02-2023
      ...
      //this.trainerRoleid = 47;          // By anif on 21-02-2023
  ...
  loadDatePicker() {  //By anif on 21-02-2023
  ...
  onChageBatch() {
    this.searchCandidateInfo.batchId = this.selectedBatchId;
    this.fillAccommodationDetails = [];
    this.reportingInductionVenueList = [];
    this.getCandidateInfoBatch();
  ...
  getCandidateInfoBatch() {
    ...
    this.SpinnerService.show();
    this.joinersservice.getAllInductionDetailsForBatchReassignCandidate(this.searchCandidateInfo).subscribe((result) => {
      if (result) {
        this.inductionDetailsForBatch = result;
        ...
        //console.log("Reassign Induction Details", this.reassignInductionDetails);
  ...
  changeVertical(changeType: any) {
    this.batchs = this.allBatch.filter(e => e.verticalId == this.verticalId && e.batchId >= this.batchId);
    if (changeType == "CL") {
  ...
  getAllBatch() {
    ...
    //this.commonService.getAllBatch(this.searchformScheduledBatchWise.value).subscribe((result) => {
    this.assessmentService.getAllBatchForReassign(this.searchformScheduledBatchWise.value).subscribe((result: any) => {
      if (result) {
        this.allBatch = result;
        //console.log("Batch List", this.allBatch);
        this.getAllVerticals();
      }
      else {
        this.allBatch = [];
  ...
  getAllVerticals() {
    ...
    if (this.type == "C") {
      ...
      this.verticalId = selectedVerticalObj.verticalId;
      this.changeVertical('PL');
    } else if (this.type == "P") {
  ...
  // getReportingVenueDetails() {
  //   if (this.candidateId != undefined && this.batchId == undefined) {
  //     this.searchReportingVenue.batchId = null;
  //     this.searchReportingVenue.candidateId = Number(this.candidateId);
  //   } else {
  //     this.searchReportingVenue.batchId = Number(this.batchId);
  //     this.searchReportingVenue.candidateId = null;
  //   }
  ...
  //   this.SpinnerService.show();
  //   this.joinersservice.getExistedReportingVenue(this.searchReportingVenue).subscribe((result) => {
  //     if (result) {
  //       this.reportingVenueExists = result;
  //       this.reportingVenue = result.venueId;
  //       this.reportingVenueAddress = result.reprtingVenueAddress;
  //       this.saveReportingVenueForm.candidateReprtingVenueId = result.candidateReprtingVenueId;
  //       this.saveReportingVenueForm.reprtingVenue = result.venueId == 0 ? null : result.venueId;
  //       this.saveReportingVenueForm.reprtingVenueAddress = result.reprtingVenueAddress;
  //       // console.log("Existed Reporting Venue Details", this.reportingVenueExists);
  ...
  //   // })
  //   this.reportingVenueAddress = this.venues.filter(x => x.reportingVenueId == evt)[0].reportingVenueAddress;
  // }
  ...
  //     batchId: this.reassigningFor == "Candidate" ? null : Number(this.batchId),
  //     candidateId: this.reassigningFor == "Candidate" ? Number(this.candidateId) : null,
  //     //isActive: true,
  //     uDTCandidateAccomodationDetail: [],
  //     createdBy: this.loginUserId
  //   }
  //   // update accommodation from fillaccommodation array
  //   this.editAccommodationCandidateDetails.forEach(element => {
  //     this.fillAccommodationDetails.forEach(pop_element => {
  //       if (element.fromDate == pop_element.fromDate && element.toDate == pop_element.toDate && element.candidateId == pop_element.candidateId) {
  //         element.accomodation = pop_element.accommodation;
  //       }
  //     })
  //   })
  ...
  //   })
  //   // console.log("Final Submit Accommodation Obj Reassign", obj);
  //   this.SpinnerService.show();
  ...
  onClickSubmit() {
    ...
    this.reassignInductionDetails.forEach(re_element => {
      if (re_element.isChecked) {
        let reassignCandidateInductionScheduleDetailsObj = {
          ...
          BatchCandidateIds: (re_element.batchCandidateIds == "" ? this.candidateId.toString() : (re_element.batchCandidateIds + "," + this.candidateId.toString()))
        }
    ...
    this.SpinnerService.show();
    this.joinersservice.trainingReassignForBatchCanndidate(finalObj).subscribe((result) => {
      if (result) {
        if (result.successFlag == 0) {
          this.SpinnerService.hide();
          this.notificationService.showError(result.msg, "Error");
  ...
  //   if (this.saveReportingVenueForm.reprtingVenueAddress == "") {
  //     flag = 1;
  ...
  //   }
  //   if (this.saveReportingVenueForm.reprtingVenue == null) {
  //     flag = 1;
  ...
  //     this.SpinnerService.show();
  //     if (this.candidateId != undefined && this.batchId == undefined) {
  //       this.saveReportingVenueForm.candidateId = Number(this.candidateId);
  //       this.saveReportingVenueForm.batchId = null;
  //     } else {
  //       this.saveReportingVenueForm.candidateId = null;
  //       this.saveReportingVenueForm.batchId = Number(this.batchId);
  //     }
  //     // console.log("reporting venue", this.saveReportingVenueForm);

  //     this.joinersservice.addReportingVenue(this.saveReportingVenueForm).subscribe((result) => {
  //       if (result) {
  //         if (result.successFlag == 0) {
  //           this.SpinnerService.hide();
  //           this.notificationService.showError(result.msg, "Error");
  ...
  // Added from here By anif on 14-01-2023
  ...
  onChangeLocation(data: any) {
    ...
    this.getAllTicByLocation(this.locationID);    // By Anif on 21-02-2023
  }
  onChangeVenue(data) {
  ...
  onChangeCoordinator() {
    if (this.coordinatorID != undefined) {
      ...
      var coordinatorNameObj = this.trainingInchargeList.find(e => e.autoUserId == this.coordinatorID);    // By Anif on 21-02-2023
  ...
  onFinalSubmit() {
    if (this.inductionScheduleDetailsNonEditableList.length > 0) {
      let finalSubmitObj = {
        candidateInductionScheduleId: 0,
        batchId: this.fromBatch ? this.selectedBatchId : null,
        candidateId: this.fromBatch ? null : Number(this.candidateId),
        createdBy: this.loginUserId,
      ...
      this.joinersservice.reassignCandidateNewScheduleInduction(finalSubmitObj).subscribe((result) => {
        if (result) {
          if (result.successFlag == 0) {
            this.SpinnerService.hide();
            this.notificationService.showError(result.msg, "Error");
  ...
  onClickBack() {   // By Anif on 21-02-2023 
    this.showNewAddSection = false;
  }
  ...
  getAllTicByLocation(locationId) {    // By anif on 21-02-2023
    this.trainingInchargeList = [];
    //this.searchTic.locationId = Number(this.locationID);
    this.searchTic.locationId = Number(locationId);          // By Anif on 21-02-2023
  ...
  onDeleteConfirm() {
    ...
    this.joinersservice.DeleteCandidateInductionScheduleDetails(deleteObj).subscribe((result) => {
      if (result.successFlag == 0) {
        this.notificationService.showError(result.msg, "Error");
...
class EditSchedule {     // By Anif on 21-02-2023
