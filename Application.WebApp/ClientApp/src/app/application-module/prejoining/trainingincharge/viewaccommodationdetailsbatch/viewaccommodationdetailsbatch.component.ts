...
import {
  IReportingVenue, IEditAccommodationBatch, IEditAccomodationForCandidateBatch, IEditAccomodationInductionSheduleBatch, IAccommodationDetailsByCandidate,
  IGetEditCandidate, IGetEditAccomodationForCandidate, IEditAccomodationCandidateBatch, ITrainingEditAccomodationRequiredCandidate
...
@Component({
  selector: 'app-viewaccommodationdetailsbatch',
  templateUrl: './viewaccommodationdetailsbatch.component.html',
  styleUrls: ['./viewaccommodationdetailsbatch.component.css']
})
export class ViewaccommodationdetailsbatchComponent implements OnInit {
  ...
  editAccommodationDetails: IEditAccommodationBatch;
  editAccommodationcandidateDetailsObj: IEditAccomodationCandidateBatch;
  editAccommodationInductionSchedule: IEditAccomodationInductionSheduleBatch[] = [];
  editAccommodationCandidateDetails: IEditAccomodationForCandidateBatch[] = [];
  ...
  getAllDetailsForEdit() {
    this.joinersservice.getAllBatchDetailsForViewAccommodation(this.searchEditAccommodationForm.value).subscribe((result) => {
      if (result) {
        this.editAccommodationDetails = result;
        this.editAccommodationInductionSchedule = this.editAccommodationDetails.trainingEditAccomodationInductionSheduleDetailsBatch; // 1st grid
        this.editAccommodationcandidateDetailsObj = this.editAccommodationDetails.editAccomodationCandidateDetailBatch;
        this.editAccommodationCandidateDetails = this.editAccommodationDetails.trainingEditAccomodationForCandidateDetailsBatch; // 2nd grid
        this.trainingEditAccomodationRequiredCandidate = this.editAccommodationDetails.trainingEditAccomodationRequiredCandidate;
  ...
  modifyAllValue() {
    ...
    if (this.filteredArrayForShowingAllDatesLocation.length > 0) {
      this.filteredArrayForShowingAllDatesLocation.forEach(element => {
        //var dateObj = this.filteredArrayForShowingDatesLocation.find(e => e.fromDate == element.fromDate && e.toDate == element.toDate && e.locationId == element.locationId); //commented out on 16-08-2023 for not showing accomodationdetails properly by Amatya
        var dateObj = this.filteredArrayForShowingDatesLocation.find(e => e.fromDate == element.dateFrom && e.toDate == element.dateTo && e.locationId == element.location);
        if (dateObj == null) {
  ...
  getAccommodationDetails(candidateDetails, dateDetails) {
    ...
    if (candidatesList.includes(candidateDetails.candidateId.toString()) && candidateDetails.isAccomadateRequired) {
      //return candidateDetails.accomodation;
      var candidateAccommodationDetailObj = this.editAccommodationCandidateDetails.find(e => e.candidateId == candidateDetails.candidateId && e.fromDate == dateDetails.dateFrom && e.toDate == dateDetails.dateTo && e.locationId == dateDetails.location);
      if (candidateAccommodationDetailObj != undefined) {
        var candidateAccommodationDetailsId = candidateAccommodationDetailObj.candidateAccomodationDetailsId;
      }
      var accommodationdetails = this.editAccommodationCandidateDetails.find(e => e.candidateId == candidateDetails.candidateId && e.fromDate == dateDetails.dateFrom && e.toDate == dateDetails.dateTo && e.candidateAccomodationDetailsId == candidateAccommodationDetailsId);
      if (accommodationdetails != undefined) {
        return (accommodationdetails.accomodation == null && !accommodationdetails.isAccomadateRequired) ? "Not requiredd" : (accommodationdetails.accomodation == null && accommodationdetails.isAccomadateRequired) ? "Pending" : accommodationdetails.accomodation;
      } else {
        return "Not Requireddd";
      }

    }
    else {
      return "Not Requiredr";
    }
  }
