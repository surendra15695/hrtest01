...
import { ICandidateInductionSchedule, ICandidateInductionSheduleHeader, ICandidateInductionSheduleDetails } from '../../../../../interfaces/prejoining/onboardingcoordinator.interface';
...
@Component({
  selector: 'app-occorporatejoinerslist',
  templateUrl: './occorporatejoinerslist.component.html',
  styleUrls: ['./occorporatejoinerslist.component.css']
})
export class OccorporatejoinerslistComponent implements OnInit {
  ...
  createJoinerListSearchForm() {
    this.searchformJoinerList = this.fb.group({
      ...
      dtofJoiningTo: [''],
      candidateName: [''],
      // verticalId: [{ value: 1, disabled: true }],
      verticalId: [1],
      locationId: [0],
      functionId: [0],
    });
  ...
  getAllDocumentName() {
    ...
    this.attachmentDocumentService.getAllDocumentName(this.searchDocumentName).subscribe((result) => {
      if (result) {
        ...
        // console.log("Joiing Document List", this.documentNames);
      }
      else {
        this.documentNames = [];
      }
  ...
  getPendingScheduleListindividually() {
    ...
    this.joinersservice.getAllPendingScheduleCandidateListIndividual(this.searchformPendingScheduleIndividual.value).subscribe((result) => {
      if (result) {
        // this.pendingScheduleCandidateindividual = result.filter(x => x.verticalId == 1);
  ...
  getAllScheduledListIndividually() {
    ...
    this.joinersservice.getAllScheduledIndividually(this.searchformScheduledIndividual.value).subscribe((result) => {
      if (result) {
        // this.scheduledIndividually = result.filter(x => x.verticalId == 1);
        this.scheduledIndividually = result;
        this.scheduledIndividually = this.scheduledIndividually.filter(x => x.onBoardingCoordinator == this.loginUserId); // no need as vertical will come from filter        
        this.scheduledIndividually.forEach(element => {
          if (element.onboardingCordinatorNotApproveDoc != " ") {
            element.popoverContent = "<div><span class='grey'>Pending/Need clarification Document: </span><span>" + element.onboardingCordinatorNotApproveDoc + "</span></div>";
          }
  ...
  onChangeBatchTtype(value) {
    if (value == "N") {
      ...
      // this.showBatchPrefix = true;
    } else {
      this.confirmJoiningTypeForm.controls.batchId.enable();
      // this.showBatchPrefix = false;
    }
  }
  ...
  onClickViewCandidate(data, pagetext, parentTabName, childTabName) {
    ...
    this._route.navigate(['/app/viewcandidatedetails'], { queryParams: { BatchId: data.batchId, BatchNo: data.batchNo, OnBoardingcoordinator: data.onBoardingcoordinator, From: "OC", Type: "C" } });
  }
  ...
  onClickIndividualScheduleInduction(data) {
    ...
    this._route.navigate(['/app/scheduleinduction'], { queryParams: { InductionType: "Candidate", IndictionTypeId: data.candidateId, IndictionTypeNo: data.candidateNo, InductionJoiningType: "I", From: "Corporate", Mode: "Add", UserType: "OC" } });
  }
  onClickIndividualEditSchedule(data) {
    jQuery(".custom-menu").hide();
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Individual");
    this._route.navigate(['/app/scheduleinduction'], { queryParams: { InductionType: "Candidate", IndictionTypeId: data.candidateId, IndictionTypeNo: data.candidateNo, InductionJoiningType: "I", From: "Corporate", CandidateInductionScheduleId: data.candidateInductionScheduleId, Mode: "Edit", UserType: "OC" } });
  }
  onClickIndividualViewSchedule(data) {
    jQuery(".custom-menu").hide();
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Individual");
    this._route.navigate(['/app/scheduleinduction'], { queryParams: { InductionType: "Candidate", IndictionTypeId: data.candidateId, IndictionTypeNo: data.candidateNo, InductionJoiningType: "I", From: "Corporate", CandidateInductionScheduleId: data.candidateInductionScheduleId, Mode: "View", UserType: "OC" } });
  }
  onClickBatchWiseInductionSchedule(data) {
    jQuery(".custom-menu").hide();
    this.persistance.set('parentActiveTab', "Pending Scheduled");
    this.persistance.set('childActiveTab', "Batch");
    this._route.navigate(['/app/scheduleinduction'], { queryParams: { InductionType: "Batch", IndictionTypeId: data.batchId, IndictionTypeNo: data.batchNo, InductionJoiningType: "B", From: "Corporate", Mode: "Add", UserType: "OC" } });
  }
  onClicEditScheduleBatchWise(data) {
    jQuery(".custom-menu").hide();
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Batch");
    this._route.navigate(['/app/scheduleinduction'], { queryParams: { InductionType: "Batch", IndictionTypeId: data.batchId, IndictionTypeNo: data.batchNo, InductionJoiningType: "B", From: "Corporate", CandidateInductionScheduleId: data.candidateInductionScheduleId, Mode: "Edit", UserType: "OC" } });
  ...
  onClicViewScheduleBatchWise(data) {
    ...
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Batch");
    this._route.navigate(['/app/scheduleinduction'], { queryParams: { InductionType: "Batch", IndictionTypeId: data.batchId, IndictionTypeNo: data.batchNo, InductionJoiningType: "B", From: "Corporate", CandidateInductionScheduleId: data.candidateInductionScheduleId, Mode: "View", UserType: "OC" } });
  ...
  // onClickEditReportingVenue(data, type) {
  //   this.addingReportingVenueFor = type;
  //   if (type == "Candidate") {
  //     this.addingReportingVenueForNo = data.candidateNo; //data.candidateNo
  //     this.addingReportingVenueForId = data.candidateId;
  ...
  onClickBookAccommodationIndividual(data) {
    ...
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Individual");
    this._route.navigate(['/app/scheduleaccommodation'], { queryParams: { AccommodationFor: "Candidate", AccommodationForId: data.candidateId, AccommodationForNo: data.candidateNo, From: "Corporate", UserType: "OC" } });
  }
  onClickBookAccommodationBatchwise(data) {
    jQuery(".custom-menu").hide();
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Batch");
    //this._route.navigate(['/scheduleaccommodation'], { queryParams: { AccommodationFor: "Batch", AccommodationForId: data.batchId, AccommodationForNo: data.batchNo, From: "Corporate", UserType: "OC" } });
    this._route.navigate(['/app/scheduleaccommodationbatch'], { queryParams: { AccommodationFor: "Batch", AccommodationForId: data.batchId, AccommodationForNo: data.batchNo, From: "Corporate", UserType: "OC" } });
  }
  // Edit Accommodation
  ...
  onClickEditAccommodationIndividual(data) {
    ...
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Individual");
    //this._route.navigate(['/editaccommodation'], { queryParams: { EditAccommodationFor: "Candidate", EditAccommodationForId: data.candidateId, EditAccommodationForNo: data.candidateNo, InductionScheduleId: data.candidateInductionScheduleId, From: "Corporate" } });
    this._route.navigate(['/app/editaccommodation'], { queryParams: { EditAccommodationFor: "Candidate", EditAccommodationForId: data.candidateId, EditAccommodationForNo: data.candidateNo, InductionScheduleId: data.candidateInductionScheduleId, From: "Corporate", UserType: "OC" } });
  }
  onClickEditAccommodationBatchWise(data) {
    jQuery(".custom-menu").hide();
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Batch");
    //this._route.navigate(['/editaccommodation'], { queryParams: { EditAccommodationFor: "Batch", EditAccommodationForId: data.batchId, EditAccommodationForNo: data.batchNo, InductionScheduleId: data.candidateInductionScheduleId, From: "Corporate", UserType: "OC" } });
    this._route.navigate(['/app/editaccommodation-batch'], { queryParams: { EditAccommodationFor: "Batch", EditAccommodationForId: data.batchId, EditAccommodationForNo: data.batchNo, InductionScheduleId: data.candidateInductionScheduleId, From: "Corporate", UserType: "OC" } });
  }
  ...
  changeTestEmailTemplate() {
    ...
    if (this.sharingType == "A") {
      ...
      this.joinersservice.getAllDetailsForEditAccommodation(this.searchAccommodationDetails).subscribe((result) => {
        ...
        this.accommodationDetails = this.accommodationAndVenueDetails.trainingEditAccomodationForCandidateDetails;
        this.reportingTimeDetails = this.accommodationAndVenueDetails.trainingEditAccomodationInductionSheduleDetails;
        // console.log("Accommodation adn Induction Details", this.accommodationAndVenueDetails);
        ...
        var cFlag = 0;
        // console.log("Batch Wise candidate List", this.batchWiseCandidateList);
      ...
      this.candidateService.getCandidateInductionPlan(serachInductionAndAccomodationDetails).subscribe((result) => {
        if (result) {
          ...
          if (this.candidateInductionPlan.candidateInductionPlanShedule != null) {
            ...
            //this.inductionDetails.batchNo = this.candidateInductionPlan.candidateInductionPlanShedule.batchId == null ? "" : this.candidateInductionPlan.candidateInductionPlanShedule.batchNo;
    ...
    this.candidateService.getCandidateInductionPlan(serachInductionAndAccomodationDetails).subscribe((result) => {
      if (result) {
        ...
        if (this.candidateAllAccommodationDetails.length > 0) {
          this.candidateAllAccommodationDetails.forEach(element => {
            var existed = this.accomodationDetails.accommodationList.find(e => e.fromDate == element.fromDate && e.toDate == element.toDate);
            //var existed = this.accomodationDetails.accommodationList.find(e => e.candidateInductionScheduleDetailsId==element.candidateInductionScheduleDetailsId);
            if (existed == null) {
              var obj = {
                fromDate: element.fromDate,
                toDate: element.toDate,
        ...
        // console.log("Accommodation Details", this.accommodationData);
        // Prepare Induction Data
        ...
        if (this.candidateInductionPlan.candidateInductionPlanShedules.length > 0) {
          if (this.candidateInductionPlan.candidateInductionPlanAccomodations.length > 0) {  // this is added  on 31-12-2022 to filter induction plan
            this.candidateInductionPlan.candidateInductionPlanAccomodations.forEach((acc_element, index) => {
              this.candidateInductionPlan.candidateInductionPlanShedules.forEach(element => {
                if (acc_element.candidateInductionScheduleDetailsId == element.candidateInductionScheduleDetailsId) {
                  // var existed = this.inductionDetails.inductionList.find(e => e.trainingTittle == element.traingTitle);
                  var existed = this.inductionDetails.inductionList.find(e => e.trainingTittleId == element.trainingTittleId);
                  if (existed == null) {
                    let obj = {
                      trainingTittle: element.traingTitle,
                      trainingTittleId: element.trainingTittleId,
        ...
        this.inductionData = this.inductionDetails;
        // console.log("Induction Details", this.inductionData);
  ...
  prepareDataForMultiSelectCandidate(CandidateDetails: any) {
    ...
    this.candidateService.getCandidateInductionPlan(serachInductionAndAccomodationDetails).subscribe((result) => {
      if (result) {
        ...
        // console.log("Test Data", this.saveShareWithCandidate.shareWithCandidatesInductionAccommodationAttachment);
    ...
    if (flag == 0) {
      ...
      if (this.sharingType == "A") {
        this.saveShareWithCandidate.shareWithCandidates.forEach(can_element => {
          ...
          //   this.allAccommodationDetails += acc_element.accomodation + ","
          // })
        ...
        this.saveShareWithCandidate.shareWithCandidates.forEach(can_element => {
          ...
          can_element.templateBody = templatedescription;
          //can_element.candidateJoiningDocumentId = this.joiningDocumentsIds;
          can_element.candidateJoiningDocumentId = "17,18";
        })
        this.saveShareWithCandidate.candidateId = this.multipleCandidateId;
        // Update template body in attachment array     // added by anif on 08-11-2022
        this.saveShareWithCandidate.shareWithCandidatesInductionAccommodationAttachment.forEach(element => {
      ...
      this.SpinnerService.show();
    //  console.log("Share with candiadte data", this.saveShareWithCandidate);
    ...
    if (flag == 0) {
      ...
      if (this.sharingType == "A") {
        this.saveShareWithCandidate.shareWithCandidates.forEach(can_element => {
          ...
          this.trainingCoordinatorDetails = "We advise you to get in touch with ";
          var venuDetail = this.allCandidateVenueDetails.find(e => e.candidateId == can_element.candidateId);
          //accommodationDetails = this.identicalAccommodationDetails.filter(e => e.candidateId == can_element.candidateId);
        ...
        if (this.saveShareWithCandidate != undefined && this.saveShareWithCandidate.shareWithCandidates.length > 0) {
          this.saveShareWithCandidate.shareWithCandidates.forEach((element_prev => {
            ...
            // var docIds = element_prev.candidateJoiningDocumentId.split(",");  // previous
            var docIds = [];
            if (this.candidateAndAdditionalDocIds.length > 0) {           // Added on 25-01-2023
              docIds = this.candidateAndAdditionalDocIds.filter(e => e.candidateId == element_prev.candidateId);
            }
        ...
        this.saveShareWithCandidate.shareWithCandidates.forEach(can_element => {
          ...
          templatedescription = templatedescription.replace("@@Coordinator", venuDetail.onBoardingCoordinatrDetaills); //changed on 16-11-2023 for showing only oc
          can_element.templateBody = templatedescription;
          can_element.templateBody = templatedescription;
          can_element.candidateJoiningDocumentId = this.joiningDocumentsIds;
        })
  ...
  onClickJoiningCheckList(data) {
    ...
    this.persistance.set('parentActiveTab', "Scheduled");
    this.persistance.set('childActiveTab', "Individual");
    this._route.navigate(['/app/ocjoiningchecklist'], { queryParams: { CandidateId: data.candidateId, CandidateNo: data.candidateNo, From: "OC", Type: "C", HiringStatus: data.hiringStatusName } });
  }
  ...
  getAllScheduleDetails(data: any, shareBy: string) {
    ...
    this.joinersservice.getScheduleInductionDetails(this.searchInductionSchedule).subscribe((result) => {
      if (result) {
        ...
        if (shareBy == "Batch") {
          // Share details batch wise
          this.shareWithInductorsBatchWise(data);
        } else {
          // share details candidate wise
  ...
  //   jQuery(".custom-menu").hide();
  //   this.persistance.set('pagename', "occorporatejoinerslist");
  ...
  //   this._route.navigate(['/corporate/new-joiner-list/occorporatejoinerslist']);
  // }
  gotoCandidateAction(id: any, requisitionDetailId: any) {
    ...
    //   this.persistance.set('candidateid', id);
    //   this.persistance.set('paramid', requisitionDetailId);
    //   this._route.navigate(['/my-action/all-positions/candidate-list/candidate-cmd-approval']);
    // }
  ...
  onClickReassignCandidate(data, parentTabName, childTabName) {
    ...
    this.persistance.set('parentActiveTab', parentTabName);
    this.persistance.set('childActiveTab', childTabName);
    //this._route.navigate(['/oc-reassign-candiadte'], { queryParams: { CandidateId: data.candidateId, For: "Candidate", Type: "C", CandidateInductionScheduleId: data.candidateInductionScheduleId } });
    this._route.navigate(['/app/oc-reassign-individualcandiadte'], { queryParams: { CandidateId: data.candidateId, BatchId: 0, BatchNo: "", For: "Candidate", Type: "C", ReqDetailsId: data.requisitionDetailId, EmpNo: data.empNp, VerticalID: this.verticalIds } });
  }
