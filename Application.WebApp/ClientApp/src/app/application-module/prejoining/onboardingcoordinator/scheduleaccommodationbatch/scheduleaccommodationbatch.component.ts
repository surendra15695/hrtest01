...
@Component({
  selector: 'app-scheduleaccommodationbatch',
  templateUrl: './scheduleaccommodationbatch.component.html',
  styleUrls: ['./scheduleaccommodationbatch.component.css']
})
export class ScheduleaccommodationbatchComponent implements OnInit {
  ...
  getAllBatchAccommodationDetails() {
    ...
    this.joinersservice.getAllAccommodationDetailsByBatch(this.searchAccommodationForm.value).subscribe((result) => {
      if (result) {
        this.scheduleAccomodation = result;
        //   console.log("Accommodation details", this.scheduleAccomodation);
        ...
        this.scheduledCandidateAccommodationList = this.scheduleAccomodation.candidateInductionScheduleByIndividuals; // 3rd grid candidate list
        this.filteredArrayForShowingAllDates = this.scheduleAccomodation.batchCandidatesScheduleDates; // filter with any of the candidate id for cheking the availale date

        //this.filteredArrayForShowingAllDates = _.filter(this.scheduledCandidateAccommodationList, { candidateId: this.firstCandidateId }); // filter with any of the candidate id for cheking the availale date
        this.filteredArrayForShowingAllDates.forEach(element => {
          var dateObj = this.filteredArrayForShowingDates.find(e => e.dateFrom == element.dateFrom && e.dateTo == element.dateTo && e.location == element.location);
          if (dateObj == null) {
            this.filteredArrayForShowingDates.push(element);
          }
        })
  ...
  showCheckBox(candidateArrayData, DatesArrayData) {
    var candidateIdFromDateArray = DatesArrayData.batchCandidateIds.split(",");
    return candidateIdFromDateArray.includes(candidateArrayData.candidateId.toString());
  }
  OnClickCandidateList(candidateIds: any) {
    this.candidatePreviewList = [];
    var candidateList = candidateIds.split(",");
    candidateList.forEach(element => {
      var candidateObj = this.inductionScheduleCandidateList.find(e => e.candidateId == element);
      if (candidateObj != undefined) {
        let candidateDetailsObj = {
          candidateNo: candidateObj.candidateNo,
          candidateName: candidateObj.candidateFullName,
        }
