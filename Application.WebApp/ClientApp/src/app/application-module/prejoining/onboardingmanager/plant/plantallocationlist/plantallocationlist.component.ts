...
@Component({
  selector: 'app-plantallocationlist',
  templateUrl: './plantallocationlist.component.html',
  styleUrls: ['./plantallocationlist.component.css']
})
export class PlantallocationlistComponent implements OnInit {
  ...
  loginUserId: number;
  verticalIds: string;
  plantAllocationPendingList: IPlantAllocationList[] = [];
  plantAllocationAllocatedList: IPlantAllocationList[] = [];
  ...
  allocatedCandidateId: any;
  OnBoardingCoordinator: number = null;
  ...
  getCorporateAllocationPendingList() {
    this.SpinnerService.show();
    this.plantservice.getPlantAllocationCandidateList(this.searchformPending.value).subscribe((result) => {
      if (result) {
        this.plantAllocationPendingList = result;
        this.plantAllocationPendingList = _.filter(this.plantAllocationPendingList, { onBoardingCoordinator: 0 });

        this.plantAllocationPendingList.forEach(element => {
        ...
        this.plantAllocationPendingList = [];
        this.SpinnerService.hide();
      }
    }, error => {
      console.log(error);
  ...
  getCorporateAllocationAllocatedList() {
    this.SpinnerService.show();
    this.plantservice.getPlantAllocationCandidateList(this.searchformAllocated.value).subscribe((result) => {
      if (result) {
        this.plantAllocationAllocatedList = result;
        this.plantAllocationAllocatedList = this.plantAllocationAllocatedList.filter(e => e.onBoardingCoordinator != 0);

        this.plantAllocationAllocatedList.forEach(element => {
          if (element.onboardingManagerNotApproveDoc != " ") {
            element.popoverContent = "<div><span class='grey'>Pending/Need clarification Document: </span><span>" + element.onboardingManagerNotApproveDoc + "</span></div>";
          }
        })
        ...
        this.plantAllocationAllocatedList = [];
        this.SpinnerService.hide();
      }
    }, error => {
      console.log(error);
  ...
  onCheckSelectAllPending(eve) {
    if (this.plantAllocationPendingList.length > 0) {
      var firstHiringStatusId = this.plantAllocationPendingList[0].hiringStatusId;
      var flag = 0;
      this.plantAllocationPendingList.forEach(element => {
  ...
  onCheckSelectAllAllocated(eve) {
    if (this.plantAllocationAllocatedList.length > 0) {
      var firstHiringStatusId = this.plantAllocationAllocatedList[0].hiringStatusId;
      var firstCoordinator = this.plantAllocationAllocatedList[0].onBoardingCoordinator;
      var flag = 0;
      this.plantAllocationAllocatedList.forEach(element => {
  ...
  GetSelectedHiringStatusPending(NewRow) {
    var AlredyChecked = this.plantAllocationPendingList.find(e => e.checked);
  ...
  onCheckRowWiseAllocated(data, eve, index) {
    // this.allocatedCandidateId = index.toString();
    // this.OnBoardingCoordinator = data.onBoardingCoordinator;
  ...
  GetSelectedHiringStatusAllocated(NewRow) {
    var AlredyChecked = this.plantAllocationAllocatedList.find(e => e.checked);
    if (AlredyChecked == null) {
      return true;
    } else {
  ...
  showAllocateCandidateButton() {
    var checkedObj = this.plantAllocationPendingList.find(e => e.checked == true); /* && e.hiringStatusId == 32 */
    return checkedObj == null ? false : true;
  }
  showReallocateCandidateButton() {
    var checkedObj = this.plantAllocationAllocatedList.find(e => e.checked == true); /* && e.hiringStatusId == 32 */
    return checkedObj == null ? false : true;
  }
  // showAllocationAllocateCandidateButton() {
  //   var checkedObj = this.plantAllocationAllocatedList.find(e => e.checked == true); /* && e.hiringStatusId == 32 */
  //   return checkedObj == null ? false : true;
  // }
  ...
  reallocateAllCandidate() {
    ...
    this.createReallocateForm();
    this.getAllOnboardingCoordinator();
    var onboardingCoordinatorIdForEdit = this.plantAllocationAllocatedList.find(e => e.checked == true).onBoardingCoordinator;
    this.saveformEdit.patchValue({
  ...
  //   this.plantservice.getAllOnboardingCorordinator(this.searchCoordinator).subscribe((result) => {
  //     if (result) {
  //       this.onBoardingCoordinatorList = result;
  //     }
  //     else {
  ...
  onAllocateCandidateToCoordinator() {
    ...
    if (this.allocationType == "S") {
      dateofJoining.push(this.plantAllocationPendingList[0].dateofJoining)
    }
    var obj = {}
    ...
    if (this.saveform.value.onBoardingCoordinator == null) {
      ...
      this.SpinnerService.show();
      //console.log("Piu", obj)
      // this.plantservice.sendToAllocate(this.saveform.value).subscribe((result) => {
      this.plantservice.sendToAllocate(obj).subscribe((result) => {
        if (result) {
  ...
  onReallocateCandidateToCoordinator() { // Added 29-09-2022
    ...
    if (this.reallocationType == "S") {
      dateofJoining.push(this.plantAllocationAllocatedList[0].dateofJoining)
    }
    var obj = {}
    if (this.saveformEdit.value.onBoardingCoordinator == null) {
      ...
      this.SpinnerService.show();
      //console.log("Piu", obj)
      //this.plantservice.sendToAllocate(this.saveformEdit.value).subscribe((result) => {
      this.plantservice.sendToAllocate(obj).subscribe((result) => {
        if (result) {
          if (result.successFlag == 0) {
            this.SpinnerService.hide();
  ...
  onUpdateJoiningDate() {
    if (this.dtOfJoining.nativeElement.value == "") {
      ...
      let finalSubmitObj = {
        ...
        CreatedBy: this.loginUserId,
        //RMEmailId: this.plantAllocationPendingList[0].rmEmailId
        EmailId: this.plantAllocationPendingList[0].rmEmailId
      }
      this.SpinnerService.show();
