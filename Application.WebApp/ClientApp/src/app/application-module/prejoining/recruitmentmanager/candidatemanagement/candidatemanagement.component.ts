...
import { IDocumentStatus, IHiringStatus, ISearchDocumentStatus } from '../../../../interfaces/common/common.interface';
...
export class CandidatemanagementComponent implements OnInit {
  ...
  getPrejoiningCandidateList() {
    ...
    this.recruitmentmanagerService.getPendingCandidateList(this.searchform.value).subscribe((result) => {
      if (result) {
  ...
  finalSendToDoctor() {
    if (this.saveform.value.doctors[0] == 0) { //done for validation on 25-08-2023 by Amartya
  ...
  showBtnJoiningConfirmation() {
    ...
    //var flag = 0;
    var ChyeckedObj = [];
    ChyeckedObj = this.pendingCandidateList.filter(e => e.checked == true && e.hiringStatusId >= 43 && e.hiringStatusId < 45);
    if (ChyeckedObj.length > 0) {
      var medicalCollectionId = ChyeckedObj.find(e => e.checked == true && e.hiringStatusId >= 43 && e.hiringStatusId < 45);
      if (medicalCollectionId == null) {
        return false;
      } else {
        return true;
  ...
  showBtnUpdateJoiningDate() {

    /* Change as per Requirement */
    //var flag = 0;
    var ChyeckedObj = [];
    ChyeckedObj = this.pendingCandidateList.filter(e => e.checked == true && e.hiringStatusId > 44 && e.hiringStatusId != 52);
    if (ChyeckedObj.length > 0) {
      var medicalCollectionId = ChyeckedObj.find(e => e.checked == true && e.hiringStatusId > 44 && e.hiringStatusId != 52);
      if (medicalCollectionId == null) {
        return false;
      } else {
        return true;
  ...
  showBtnSendToOnboarding() {
    ...
    if (ChyeckedObj.length > 0) {
      ...
      if (medicalCollectionId == null) {
        ...
        ChyeckedObj.forEach(element => {
          if (checkedCount == 0) {
            this.prevVerticalId = element.verticalId;
          }
          if (this.prevVerticalId != element.verticalId) {
            flag = 1;
          }
          checkedCount += 1;
  ...
  onClickUpdateJoiningDate() {
    ...
    this.getCandidateJoningDate(candidateIdString, "");
    setTimeout(() => {
      this.loadDatePicker();
    }, 1000);
  }
  ...
  onChangeJoiningDate(data) {
    this.getCandidateJoningDate(data.candidateId, data.joiningDateStatus);
    setTimeout(() => {
      this.loadDatePicker();
    }, 1000);
  }
  ...
  onSubmitUpdateJoiningDate() {
    ...
    if (flag == 0) {
      ...
      this.SpinnerService.show();
      this.candidateService.updateJoiningDate(finalSubmitObj).subscribe((result) => {
        if (result) {
          if (result.successFlag == 0) {
            this.SpinnerService.hide();
            this.notificationService.showError(result.msg, "Error");
  ...
  onSendSingleToOnboarding(data, flag) {//Piu
    ...
    if (data.verticalId == 1) {
      this.searchRoleUser.roleId = 21
    }
    else if (data.verticalId == 2) {
      this.searchRoleUser.roleId = 22
    }
    else if (data.verticalId == 3) {
      this.searchRoleUser.roleId = 23
    }
    this.getAllOnboardingmanagerList();
