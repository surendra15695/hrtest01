...
@Component({
  selector: 'app-ocrequsitioncandidateaction',
  templateUrl: './ocrequsitioncandidateaction.component.html',
  styleUrls: ['./ocrequsitioncandidateaction.component.css']
})
export class OcrequsitioncandidateactionComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.persistance.get('pagename') != null || this.persistance.get('nextpagename') != null) {
      if ((this.persistance.get('pagename') == "occorporatejoinerslist" || this.persistance.get('pagename') == "ocplantjoinerslist" ||
      this.persistance.get('pagename') == "viewcandidatelist")) {
        this.candidateId = this.persistance.get('candidateid');
        this.requisitionDetailId = this.persistance.get('paramid');
        this.getCandidateList();
        this.getTestReult();
      } else {
        this._route.navigate(['/app/occorporatejoinerslist']);
      }
    }
    else {
      this._route.navigate(['/app/occorporatejoinerslist']);
  ...
  getCandidateRemark(){
    ...
    this.offerLetterService.getOfferLetter(searchOfferLetter).subscribe((response: any) => {
      if(response.acknowlagementRemarks.length != 0){
        console.log("Check remarks-",response.acknowlagementRemarks);
  ...
  gotoCandidateList() {
    if (this.persistance.get('pagename') == "occorporatejoinerslist") {
   ...
   else if(this.persistance.get('pagename') == "ocplantjoinerslist"){
     this.persistance.set('paramid', this.requisitionDetailId);
      this.persistance.set('pagename', "occorporatejoinerslist");
      this.persistance.set('nextpagename', null);
      this._route.navigate(['/app/plant/new-joiner-list']);
   }
   else if(this.persistance.get('pagename') == "viewcandidatelist"){
    this.persistance.set('paramid', this.requisitionDetailId);
     this.persistance.set('pagename', "occorporatejoinerslist");
     this.persistance.set('nextpagename', null);
     this._route.navigate(['/app/viewcandidatedetails']);
  }
  }
