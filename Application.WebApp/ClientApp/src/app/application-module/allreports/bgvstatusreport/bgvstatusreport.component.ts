...
@Component({
  selector: 'app-bgvstatusreport',
  templateUrl: './bgvstatusreport.component.html',
  styleUrls: ['./bgvstatusreport.component.css']
})
export class BGVStatusreportComponent implements OnInit {
  ...
  formSubmit() {
    ...
    if(flag==0){
      ...
      this.reportService.bgvStatusReport(this.searchform.value).subscribe((result) => {
      if (result) {
        this.bgvStatustList = result;
        this.loadDataTable();
        this.SpinnerService.hide();
      }
      else {
  ...
  exportToExcel(){
    if(this.bgvStatustList.length != 0){
      this.bgvStatustList.forEach((e,index)=>{
        e.serialNo = index+1;
      })
      const updatedArray: any[] = [];
      this.bgvStatustList.forEach((obj) => {
      ...
      this.excelService.ExportAsExcelFileForReport(updatedArray, 'BGV Status Report');
    }
  }
  loadSelectPicker() {
    setTimeout(() => {
