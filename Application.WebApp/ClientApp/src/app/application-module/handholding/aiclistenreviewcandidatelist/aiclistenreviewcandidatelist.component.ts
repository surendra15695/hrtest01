...
@Component({
  selector: 'app-aiclistenreviewcandidatelist',
  templateUrl: './aiclistenreviewcandidatelist.component.html',
  styleUrls: ['./aiclistenreviewcandidatelist.component.css']
})
export class AiclistenreviewcandidatelistComponent implements OnInit {
  ...
  getAllocatedCandidateList() {
    this.SpinnerService.show();
    this.chandHoldingService.getAllAICAllocatedListenList(this.searchformAllocated.value).subscribe((result) => {
      if (result) {
  ...
  gotoListenReview(data)
  {
    ...
    this.persistance.set('pagename', "aiclistenwreview");
    this._route.navigate(['/app/aic/candidate/listen-review']);
  }
