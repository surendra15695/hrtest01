...
@Component({
  selector: 'app-plantlistenreviewlist',
  templateUrl: './plantlistenreviewlist.component.html',
  styleUrls: ['./plantlistenreviewlist.component.css']
})
export class PlantlistenreviewlistComponent implements OnInit {
  ...
  getAllocationPendingList() {
    ...
    this.chandHoldingService.getApproverListenList(this.searchformPending.value).subscribe((result) => {
      if (result) {
        this.allocationPendingList = result;
        this.allocationPendingList=this.allocationPendingList.filter(x=>x.reviewStatus==1);
        this.SpinnerService.hide();
      }
      else {
  ...
  getAllocationAllocatedList() {
    this.SpinnerService.show();
    this.chandHoldingService.getApproverListenList(this.searchformAllocated.value).subscribe((result) => {
      if (result) {
        this.allocationAllocatedList = result;
        this.allocationAllocatedList=this.allocationAllocatedList.filter(x=>x.reviewStatus>1);
        this.SpinnerService.hide();
      }
      else {
  ...
  gotoReviewApproval(data) {
    ...
    this.persistance.set('listenReviewId', data.listenReviewId);
    this.persistance.set('verticald', this.verticalId);
    this.persistance.set('pagename', "listenapproval");
    this._route.navigate(['/app/candidate/listen-review-approval']);
  }
