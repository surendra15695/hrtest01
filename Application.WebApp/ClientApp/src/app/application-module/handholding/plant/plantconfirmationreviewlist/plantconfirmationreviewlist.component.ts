...
@Component({
  selector: 'app-plantconfirmationreviewlist',
  templateUrl: './plantconfirmationreviewlist.component.html',
  styleUrls: ['./plantconfirmationreviewlist.component.css']
})
export class PlantconfirmationreviewlistComponent implements OnInit {
  ...
  getAllocationPendingList() {
    ...
    //   this.searchformPending.value.allocationStatus=0
    // }
    // if(roles.includes("35")){
    //   this.searchformPending.value.allocationStatus=6
    // }
    // if(roles.includes("63")){
    //   this.searchformPending.value.allocationStatus=7
    // }
    // else{
    //   this.searchformPending.value.allocationStatus=20
    // }
    this.chandHoldingService.getApproverConfirmationList(this.searchformPending.value).subscribe((result) => {
      if (result) {
        ...
        if(this.roleIds.includes("60")){
          this.allocationPendingList=this.allocationPendingList.filter(x=>x.reviewStatus==6 || x.reviewStatus==10);
        }
        if(this.roleIds.includes("63")){
          this.allocationPendingList=this.allocationPendingList.filter(x=>x.reviewStatus==7 || x.reviewStatus==11);
        }
        this.SpinnerService.hide();
      }
      else {
  ...
  getAllocationAllocatedList() {
    ...
    this.chandHoldingService.getApproverConfirmationList(this.searchformAllocated.value).subscribe((result) => {
      if (result) {
        this.allocationAllocatedList = result;
        if(this.roleIds.includes("61")){
          this.allocationAllocatedList=this.allocationAllocatedList.filter(x=>x.reviewStatus==2 || x.reviewStatus >= 6 );
          }
          if(this.roleIds.includes("60")){
            this.allocationAllocatedList=this.allocationAllocatedList.filter(x=>x.reviewStatus==2 || x.reviewStatus ==7 || x.reviewStatus ==9 || x.reviewStatus==11);
          }
          if(this.roleIds.includes("63")){
  ...
  gotoReviewApproval(data) {
    ...
    this.persistance.set('pagename', "confirmationapproval");
    this._route.navigate(['/app/candidate/confirmation-review-approval']);
  }
}
