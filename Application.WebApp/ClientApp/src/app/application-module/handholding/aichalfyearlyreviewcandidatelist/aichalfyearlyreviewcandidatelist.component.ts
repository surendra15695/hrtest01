...
@Component({
  selector: 'app-aichalfyearlyreviewcandidatelist',
  templateUrl: './aichalfyearlyreviewcandidatelist.component.html',
  styleUrls: ['./aichalfyearlyreviewcandidatelist.component.css']
})
export class AichalfyearlyreviewcandidatelistComponent implements OnInit {
  ...
  getAllocatedCandidateList() {
    this.SpinnerService.show();
    this.chandHoldingService.getAllAICAllocatedHalfYearlyList(this.searchformAllocated.value).subscribe((result) => {
      if (result) {
        
        this.allocationAllocatedList = result.filter( e=> e.probation == "1 Year");
        this.SpinnerService.hide();
      }
      else {
  ...
  gotoHalfYearlyReview(data)
  {
    ...
    this.persistance.set('pagename', "aichalfyearlyreview");
    this._route.navigate(['/app/aic/candidate/half-yearly-review']);
  }
