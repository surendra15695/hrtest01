...
@Component({
  selector: 'app-campus-candidate-edit-profile',
  templateUrl: './campus-candidate-edit-profile.component.html',
  styleUrls: ['./campus-candidate-edit-profile.component.css']
})
export class CampusCandidateEditProfileComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (flag == 0) {
      ...
      this.candidateService.updateOnlyCampusCandidateProfile(formData).subscribe((result) => {
  ...
  getCandidate() {
    ...
    this.candidateService.getCampusCandidateDataUpdate(this.searchCandidateData).subscribe((result) => {
      if (result) {
