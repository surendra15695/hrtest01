...
@Component({
  selector: 'app-off-campus-talent-pool',
  templateUrl: './off-campus-talent-pool.component.html',
  styleUrls: ['./off-campus-talent-pool.component.css']
})
export class OffCampusTalentPoolComponent implements OnInit {
  ...
  gotoCampusLink() {
    ...
    this._route.navigate(['/app/off-campus/requsition-link']);
  }
  ...
  changeTestEmailTemplate() {
    ...
    templatedescription = templatedescription.replace("@@AcknowledgementLink", this.emailIdFront + "testacknowledgement?CandiateId=@~@CandidateId");
    setTimeout(() => {
  ...
  changeInterviewEmailTemplate() {
    if (this.selectedInterviewEmailTemplateId != undefined) {
      ...
      //templatedescription = templatedescription.replace("@@InterviewLink", this.emailIdFront + "campusoffcampus/interviewacknowledgement?CandiateId=@~@CandidateId");
  ...
  gotoInterviewHRFeedback(candidateid) {
    jQuery(".custom-menu").hide();
    this.persistance.set('pagename', "offcmviewhrfeedback");
    this.persistance.set('candidateId', candidateid);
  ...
  onUpdateFunction() {
    ...
    this.campusRequisitionService.updateOffCampusCampusCandidateVerticalFunction(data).subscribe((result) => {
      if (result) {
        if (result.successFlag == 0) {
          this.notificationService.showError(result.msg, "Error");
        }
  ...
  changeFunctionfordefine() {
    ...
    this.campusCommonService.getOffCampusFunctionwiseRequisition(obj).subscribe((response: any) => {
      if (response) {
  ...
  gotoCandidateDetail(data) {
    ...
    setTimeout(() => {
      ...
      this.persistance.set('prevpagename', "offcampustalentpool");
  ...
  openOnEditCampusCandidateModal(candiadteId) {
    jQuery(".custom-menu").hide();
    this.editapplicationformcandidateid = candiadteId;
  }
  openOnEditCampusCandidateModalforRegistration(candiadteId) {
    jQuery(".custom-menu").hide();
    this.editapplicationformcandidateid = candiadteId;
  }
  ...
  onclickcancelplacement() {
    ...
    var cancelFormData = {
      CandidateIds: ((candids.split(',')).slice(1)).join(','),
      CandidateName: ((cannames.split(',')).slice(1)).join(','),
      Institute: ((institute.split(',')).slice(1)).join(','),
      Course: ((course.split(',')).slice(1)).join(','),
      Stream: ((stream.split(',')).slice(1)).join(','),
      EmailId: ((this.testmailIds.split(',')).slice(1)).join(',')
    }
        ...
        if (result.successFlag == 0) {
          ...
          result.forEach(element => {
            let headerObj = {
              ...
              Age: element.age,
              AnyWhereinIndia: element.anyWhereinIndia,
              CampusLinkId: element.campusLinkId,
              CandidateId: element.candidateId,
              CriticalFactor: element.criticalFactor,
              Disability: element.disability == false ? "No" : "Yes",
              DisabilityDetails: element.disabilityDetails,
              Dob: element.dob,
              EmailId: element.emailId,
              ExtraCurricularActivities: element.extraCurricularActivities,
              EyeSightCorrected: element.eyeSightCorrected == false ? "No" : "Yes",
              EyeSightLeft: element.eyeSightLeft,
              EyeSightRight: element.eyeSightRight,
              FatherOccupation: element.fatherOccupation,
              Gender: element.gender,
              HealthIssue: element.healthIssue == false ? "No" : "Yes",
              HealthIssueDetails: element.healthIssueDetails,
              Height: element.height,
              HighestQualification: element.highestQualification,
              HomeTown: element.homeTown,
              JobTypePriority: element.jobTypePriority,
              LanguageKnown: element.languageKnown,
              MostPreferdBenifit: element.mostPreferdBenifit,
              MotherOccupation: element.motherOccupation,
              MotherTongue: element.motherTongue,
              MrfRealtive: element.mrfRealtive == false ? "No" : "Yes",
              NativeState: element.nativeState,
              NoofSiblings: element.noofSiblings,
              PhoneNo: element.phoneNo,
              PresentState: element.presentState,
              RealtiveMobileNo: element.realtiveMobileNo,
              RealtiveName: element.realtiveName,
              Weight: element.weight,
              WorkinginShift: element.workinginShift,
              YearsCommitments: element.yearsCommitments
