...
@Component({
  selector: 'app-hrhand-holding',
  templateUrl: './hrhand-holding.component.html',
  styleUrls: ['./hrhand-holding.component.css']
})
export class HRHandHoldingComponent implements OnInit {
  ...
  onFeedback(data)
  {
    console.log("feedback",data.candidateId)
    ...
    this.persistance.set('pagename', "hrhandholding");
  ...
  getAllocationPendingList() {
    ...
    this.chandHoldingService.getAllHandHoldingAllocationList(this.searchformPending.value).subscribe((result) => {
      if (result) {
        ...
        this.allocationPendingList = result.filter( e => e.feedbackId==0 && e.elidgbleFeedBack>0);
        this.SpinnerService.hide();
      }
      else {
