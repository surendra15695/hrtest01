...
@Component({
  selector: 'app-campusrequisition',
  templateUrl: './campusrequisition.component.html',
  styleUrls: ['./campusrequisition.component.css']
})
export class CampusrequisitionComponent implements OnInit {
  saveForm : FormGroup;
  ...
  constructor(
    ...
    console.log("role",this.persistance.get('loggedinuser'))
  }
  ...
  getAllVerticals() {
    ...
    this.verticalService.getAllCAmpusVertical(obj).subscribe((response: any) => {            
      if(response){
        this.verticals = response.filter(x=>x.verticalId==1);
  ...
  oncahngegrade()
  {
    const findfunction = this.requisitionDataDetail.find(e => e.functionId == this.selectedFunction)
    if(findfunction)
    {
      for(let i=0;i<this.requisitionDataDetail.length;i++)
      {
        if(this.selectedCollegeCategory==this.requisitionDataDetail[i].collegeCategoryId)
        {
          this.notiService.showError("This College Category for this function is already selected", "")
  ...
  getAllCourseCampusStream(){ 
    ...
    this.campusCommonService.getAllCampusCourseStream().subscribe((response: any) => {            
      if(response){
        ...
        console.log("---------------");
        console.log(this.campusCouseStreamList);
        console.log("---------------");
      }
      else{
        this.campusCourseStream=[];  
  ...
  formSubmit(){
    if(this.requisitionData.length==0){
      ...
      console.log(this.requisitionFormData);
      this.requisitionService.createCampusrequisition(this.requisitionFormData).subscribe((response: any) => {      
