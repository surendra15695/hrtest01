...
@Component({
  selector: 'app-campushrreview',
  templateUrl: './campushrreview.component.html',
  styleUrls: ['./campushrreview.component.css']
})
export class CampushrreviewComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    getAllocationPendingList() {
      ...
      this.chandHoldingService.getAllHandHoldingAllocationList(this.searchformAllocated.value).subscribe((result) => {
        if (result) {
          debugger;
          this.allocationPendingList = result.filter( e => e.probation !="6 Months" && e.reviewId==0 && e.elidgbleReview==1);
          this.SpinnerService.hide();
        }
        else {
