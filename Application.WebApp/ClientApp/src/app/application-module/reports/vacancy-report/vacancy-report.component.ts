...
export class VacancyReportComponent implements OnInit {
  ...
  fromSubmit() {
    ...
    this.reportService.vacancyReport(this.searchform.value).subscribe((result) => {
      if (result) {
        this.vacancyReportList = result;
        this.vacancyReportList.forEach(e => {
          //e.hover = "<div><span class='grey'>ReqNo: </span><span>" + e.reqno + "</span></div > ";
          e.reqnoString = "<div><span class='grey'>ReqNo: </span><span>" + e.reqno + "</span></div>";
        })
        this.loadDataTable();
        this.SpinnerService.hide();
      }
  ...
  exportToExcel() {
    if(this.vacancyReportList.length != 0){
      this.vacancyReportList.forEach((e,index)=>{
        e.serialNo = index+1;
      })
      const updatedArray: any[] = [];
      this.vacancyReportList.forEach((obj) => {
      ...
      this.excelService.ExportAsExcelFileForReport(updatedArray, 'Vacancy Report');
    }
    //this.excelService.ExportAsExcelFileForReport(this.vacancyReportList, "Vacancy Report");
  }
