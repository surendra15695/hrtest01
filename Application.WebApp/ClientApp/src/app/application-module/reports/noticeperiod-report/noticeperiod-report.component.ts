...
@Component({
  selector: 'app-noticeperiod-report',
  templateUrl: './noticeperiod-report.component.html',
  styleUrls: ['./noticeperiod-report.component.css']
})
export class NoticeperiodReportComponent implements OnInit {
  ...
  formSubmit() {
    ...
    if(flag == 0){
      this.noticePeriodReportList = [];
      this.SpinnerService.show();
      this.reportService.noticePeriodReport(this.searchform.value).subscribe((result) => {
        if (result) {
          this.noticePeriodReportList = result;
          this.loadDataTable();
          this.SpinnerService.hide();
        }
        else {
  ...
  exportToExcel() {
    if(this.noticePeriodReportList.length !=0)
    {
      const updatedArray: any[] = [];
      this.noticePeriodReportList.forEach((obj) => {
    ...
    this.excelService.ExportAsExcelFileForReport(updatedArray, "Notice Period Report");
    }
