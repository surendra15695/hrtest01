...
@Component({
  selector: 'app-recruitment-manager-requisition-report',
  templateUrl: './recruitment-manager-requisition-report.component.html',
  styleUrls: ['./recruitment-manager-requisition-report.component.css']
})
export class RecruitmentManagerRequisitionReportComponent implements OnInit {
  ...
  recruitmentManagerRequisitionList: any[] = [];
  verticalIds: string;
  ...
  exportToExcel() {
    this.recruitmentManagerRequisitionList.forEach((e,index)=>{
      e.serialNo = index+1;
    })
    const updatedArray: any[] = [];
    this.recruitmentManagerRequisitionList.forEach((obj) => {
    ...
    this.excelService.ExportAsExcelFileForReport(updatedArray, "Recruitment Manager Requisition Report");
  }
  ...
  fromSubmit() {
    ...
    if(flag == 0){
      this.recruitmentManagerRequisitionList = [];
      this.SpinnerService.show();
      this.reportService.recruitmentManagerRequisitionReport(this.searchform.value).subscribe((result) => {
        if (result) {
          //console.log(result);
          this.recruitmentManagerRequisitionList = result;
          this.loadDataTable();
          this.SpinnerService.hide();
        }
        else {
