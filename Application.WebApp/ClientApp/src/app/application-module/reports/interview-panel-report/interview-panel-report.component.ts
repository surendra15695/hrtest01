...
@Component({
  selector: 'app-interview-panel-report',
  templateUrl: './interview-panel-report.component.html',
  styleUrls: ['./interview-panel-report.component.css']
})
export class InterviewPanelReportComponent implements OnInit {
  ...
  formSubmit() {
   ...
   if(flag==0){
    ...
    this.reportService.interviewPanelReport(this.searchform.value).subscribe((result) => {
      if (result) {
        this.interviewPanelList = result;
        console.log("AAAAA", this.interviewPanelList)
        this.loadDataTable();
        this.SpinnerService.hide();
      }
      else {
  ...
  exportToExcel() {
    if(this.interviewPanelList.length != 0){
      this.interviewPanelList.forEach((e,index)=>{
        e.serialNo = index+1;
      })
      const updatedArray: any[] = [];
      this.interviewPanelList.forEach((obj) => {
      ...
      this.excelService.ExportAsExcelFileForReport(updatedArray, 'Interview Panel Report');
    }
  }
