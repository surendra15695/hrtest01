...
@Component({
  selector: 'app-preemployeement-report',
  templateUrl: './preemployeement-report.component.html',
  styleUrls: ['./preemployeement-report.component.css']
})
export class PreemployeementReportComponent implements OnInit {
  ...
  formSubmit() {
    ...
    if(flag==0){
    this.preemployeementList = [];
    this.SpinnerService.show();
    this.reportService.preemployeementReport(this.searchform.value).subscribe((result) => {
      if (result) {
        this.preemployeementList = result;
        this.loadDataTable();
        this.SpinnerService.hide();
      }
      else {
  ...
  exportToExcel(){
    if(this.preemployeementList.length != 0){
      this.preemployeementList.forEach((e,index)=>{
        e.serialNo = index+1;
      })
      const updatedArray: any[] = [];
      this.preemployeementList.forEach((obj) => {
      ...
      this.excelService.ExportAsExcelFileForReport(updatedArray, 'Preemployeement Report');
    }
  }
