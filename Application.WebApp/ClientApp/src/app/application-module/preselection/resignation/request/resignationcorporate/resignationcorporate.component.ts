...
@Component({
  selector: 'app-resignationcorporate',
  templateUrl: './resignationcorporate.component.html',
  styleUrls: ['./resignationcorporate.component.css']
})
export class ResignationcorporateComponent implements OnInit {
  ...
  removeRow(index) {
    if (confirm("Are you sure to delete ")) {
      ...
      // console.log(this.requistionDetailDataArray);
      // console.log(this.requistionDetailData);
    }
  }
  loadSelectPicker() {
    setTimeout(() => {
    ...
    if (confirm("Are you sure to submit ? ")) {  //----Piu
      ...
      if (this.requistionDetailData.length == 0) {
        ...
        for (var i = 0; i < this.fileToUpload.length; i++) {
          formData.append("resignationfiles", this.fileToUpload[i].file);
        }
