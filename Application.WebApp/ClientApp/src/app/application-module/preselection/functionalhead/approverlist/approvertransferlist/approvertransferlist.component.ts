...
import { ISearchTransferList, ITransferList } from '../../../../../interfaces/preselection/transfer.interface';
...
@Component({
  selector: 'app-approvertransferlist',
  templateUrl: './approvertransferlist.component.html',
  styleUrls: ['./approvertransferlist.component.css']
})
export class ApprovertransferlistComponent implements OnInit {
  pageTitle = "Transfer - Pending Action";
  ...
  reject() {
    if (confirm("Are you sure to reject ")) {
      this.saveform.patchValue({
        TransferDetailId: this.transferDetailId,
        StatusId: 4,
        CreatedBy: this.createdBy
      })
      this.SpinnerService.show();
      this.transferService.transferApproveReject(this.saveform.value).subscribe((result) => {
        if (result) {
  ...
  changeVertical() {
    this.selectedVerticalId = this.searchform.value.verticalId;
    this.getAllLocation();
  }
