...
@Component({
  selector: 'app-requisitioncorporate',
  templateUrl: './requisitioncorporate.component.html',
  styleUrls: ['./requisitioncorporate.component.css']
})
export class RequisitioncorporateComponent implements OnInit {
  ...
  getAllFunction() {
    ...
    this.functionService.getAllLocationFunction(this.searchFunction).subscribe((result) => {
      if (result) {
        ...
        // console.log("Location Function", this.functions);
      }
      else {
        this.functions = [];
      }
  ...
  startRequisition() {
    ...
    if (flag == 0) {
      ...
      this.notificationService.showError("Please select Corporate Location !!", "Error");
    }
  }
  ...
  formSubmit() {
    if (confirm("Are you sure to submit ? ")) {  //----Piu
      ...
      if (this.requistionDetailData.length == 0) {
        ...
        //  console.log("Requisition Details Data", this.requistionDetailData);
