...
@Component({
  selector: 'app-off-campus-testtravelreimbursement',
  templateUrl: './off-campus-testtravelreimbursement.component.html',
  styleUrls: ['./off-campus-testtravelreimbursement.component.css']
})
export class OffCampusTesttravelreimbursementComponent implements OnInit {
  ...
  getAllTravelReimbursementList(){
    ...
    this.travelReimbursementService.getCampusCandidateTestTravelReimbursementList(this.searchTravelReimbursementList).subscribe((result) => {
      if (result) {
  ...
  gotoAddTravelReimbursement(testScheduleDetailId){
    ...
    this.persistance.set('pagename', "offcampustesttravellist");
    this._route.navigate(['/app/career/campus-add-test-travel-reimbursement']);
  }
